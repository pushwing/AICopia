...
기업형 홈페이지(페이지·게시판·문의)에 쇼핑몰 기능을 더한 단일 CI4 프로젝트입니다. 저장소 루트가 곧 하나의 CI4 프로젝트입니다.  
...
| 구분 | 값 |
|------|----|
| 이메일 | `admin@example.com` (`.env` 의 `ADMIN_INITIAL_EMAIL` 로 변경 가능) |
| 비밀번호 | **`php spark migrate` 실행 시 콘솔에 1회 출력** (설치마다 랜덤 생성) |

> 초기 비밀번호는 랜덤으로 생성되며 마이그레이션 출력에만 나옵니다. 최초 로그인 시
> 비밀번호 변경이 강제됩니다.
