...
> ## ✅ 2026-08-08 운영 적용 완료
>
> 아래 절차를 운영 DB 에 적용해 `migrate --all` 이 `Migrations complete.` 로 통과하는 것을 확인했다.
> **이미 처리된 건이므로 다시 실행할 필요가 없다.**
>
> 적용 전 확인값 — `ci_settings.context` 존재(1) / `settings.context` 없음(0) /
> `AddContextColumn` 기록 없음(0) / `CreateSettingsTable` 기록 있음(1).
>
> 이 문서는 **같은 유형의 드리프트가 다시 생겼을 때의 참고 절차**로 남긴다.
> vendor 라이브러리 마이그레이션과 앱 마이그레이션이 같은 테이블을 건드리면
> 언제든 재발할 수 있다.
...
드리프트 자체는 당장 장애를 일으키지 않는다. 다만 **스키마가 바뀌는 배포에서 반드시 막힌다.**

배포 워크플로는 이제 마이그레이션 실패를 배포 실패로 잡으므로(#134), 예전처럼 조용히 넘어가 "컬럼 없는 DB 위에 새 코드" 상태가 되지는 않는다. 대신 **배포 자체가 중단된다.** 급한 보안 패치를 배포해야 하는 순간에 막히는 게 최악이므로, 발견하면 여유 있을 때 바로 정리한다.

### 이 문제가 생긴 배경 (2026-08-08 사고)

`#133` 배포에서 `migrate --all` 이 이 드리프트로 중단됐는데, `spark` 가 예외를 내고도 종료코드 0 을 돌려주는 바람에 **배포가 `success` 로 집계됐다.** 그 결과 뒤따르던 앱 마이그레이션(`AddMustChangePassword`)이 적용되지 않은 채 새 코드만 올라가 비밀번호 변경 기능이 깨졌다. 운영에서 App 네임스페이스만 수동 마이그레이션해 복구했고, 이후 `#134` 로 워크플로가 실패를 잡도록 고쳤다.

교훈: **마이그레이션이 포함된 배포는 워크플로의 성공 표시만 믿지 말고 실제 마이그레이션 출력을 확인한다.**
