...
final class AdminWithdrawnUserTest extends CIUnitTestCase
{
    ...
    /**
     * GradeService::getGoldMembersForPromotion() — 탈퇴한 골드회원은
     * 플래티넘 승급 후보 화면에 노출되면 안 된다(최종 리뷰 결함 #3).
     */
    public function testGoldMembersForPromotionExcludesWithdrawnUser(): void
    {
        $keptId = $this->insertUser('gold');
        $goneId = $this->insertUser('gold');
        $this->withdraw($goneId);

        $result = new GradeService()->getGoldMembersForPromotion();

        $ids = array_map('intval', array_column($result['items'], 'id'));
        $this->assertContains($keptId, $ids);
        $this->assertNotContains($goneId, $ids, '탈퇴한 골드회원이 플래티넘 승급 후보에 섞이면 안 된다');
    }

    /**
     * PointController::index() 가 쓰는 것과 동일한 쿼리 조건
     * (role='member' AND withdrawn_at IS NULL) — 탈퇴회원의 포인트 잔액이
     * 관리자 포인트 목록에 남으면 안 된다(최종 리뷰 결함 #3).
     */
    public function testPointListQueryConditionExcludesWithdrawnUser(): void
    {
        $keptId = $this->insertUser();
        $goneId = $this->insertUser();
        $this->withdraw($goneId);
        ...
        $ids = array_map(
            'intval',
            array_column(
                db_connect()->table('users')->select('id')
                    ->where('role', 'member')
        ...
        $this->assertContains($keptId, $ids);
        $this->assertNotContains($goneId, $ids, '탈퇴회원이 관리자 포인트 목록 쿼리에 섞이면 안 된다');
    }

    /**
     * DashboardController 의 total_users 통계 — UserModel::activeBuilder()
     * 를 쓰지 않으면 탈퇴회원까지 카운트에 섞인다(최종 리뷰 결함 #3).
     */
    public function testTotalUsersCountExcludesWithdrawnUser(): void
    {
        $before = new UserModel()->activeBuilder()->countAllResults();

        $keptId = $this->insertUser();
        $goneId = $this->insertUser();
        $this->withdraw($goneId);

        $after = new UserModel()->activeBuilder()->countAllResults();

        $this->assertSame($before + 1, $after, 'active 카운트는 신규 회원 1명만 늘어야 한다(탈퇴회원은 제외)');
    }
}
